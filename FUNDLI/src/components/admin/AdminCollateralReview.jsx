...
const AdminCollateralReview = () => {
  ...
  const loadAllVerifications = async () => {
    try {
      ...
      const pendingResponse = await fetch(buildApiUrl('/collateral/admin/pending'), {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      
      // Load approved verifications
      const approvedResponse = await fetch(buildApiUrl('/collateral/admin/approved'), {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      
      // Load deleted verifications
      const deletedResponse = await fetch(buildApiUrl('/collateral/admin/deleted'), {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(buildApiUrl(`/collateral/admin/${selectedVerification.id}/review`), {
        method: 'POST',
        headers: {
  ...
  const handleDeleteVerification = async () => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(buildApiUrl(`/collateral/admin/${selectedVerification.id}/delete`), {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'submitted': return 'bg-warning/20 text-warning dark:bg-warning/20 dark:text-warning/50';
      case 'under_review': return 'bg-primary-100 text-primary-800 dark:bg-primary-900/20 dark:text-primary-400';
      case 'approved': return 'bg-success/20 text-success dark:bg-success/20 dark:text-success/50';
      case 'deleted': return 'bg-neutral-100 text-neutral-800 dark:bg-secondary-900/20 dark:text-neutral-400';
    }
  };
    ...
    <div className="max-w-7xl mx-auto p-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 border border-neutral-300 dark:border-neutral-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-primary-500 dark:bg-neutral-700 dark:text-white"
                />
              </div>
            </div>
            <div className="flex items-center space-x-2">
              ...
              <select
                ...
                className="px-3 py-2 border border-neutral-300 dark:border-neutral-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-primary-500 dark:bg-neutral-700 dark:text-white"
              >
        ...
        <div className="border-b border-neutral-200 dark:border-secondary-700">
          <nav className="-mb-px flex space-x-8">
            ...
            <button
              ...
              className={`py-2 px-1 border-b-2 font-medium text-sm ${
                activeTab === 'approved'
                  ? 'border-success text-success dark:text-success/50'
                  : 'border-transparent text-neutral-500 hover:text-neutral-700 hover:border-neutral-300 dark:text-neutral-400 dark:hover:text-neutral-300'
              }`}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white dark:bg-secondary-800 rounded-lg shadow-sm border border-neutral-200 dark:border-secondary-700">
            <div className="p-4 border-b border-neutral-200 dark:border-secondary-700 flex justify-between items-center">
              ...
              <button
                ...
                disabled={refreshing}
                className="p-2 text-neutral-500 hover:text-neutral-700 dark:text-neutral-400 dark:hover:text-neutral-200 transition-colors disabled:opacity-50"
                title="Refresh data"
              >
                    ...
                    >
                      <div className="flex items-start justify-between">
                        ...
                        >
                          ...
                          <p className="text-sm font-medium text-success dark:text-success/50">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <div className="relative">
                            ...
                            {showActions[verification.id] && (
                              <div className="actions-menu absolute right-0 top-8 bg-white dark:bg-secondary-800 border border-neutral-200 dark:border-secondary-700 rounded-lg shadow-lg z-10 min-w-[160px]">
                                <button
                                  onClick={() => {
                                    setSelectedVerification(verification);
                                    setShowActions({});
                                  }}
                                  className="w-full px-3 py-2 text-left text-sm text-neutral-700 dark:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-neutral-700 flex items-center space-x-2"
                                >
        ...
        <div className="lg:col-span-2">
          {selectedVerification ? (
            <div className="space-y-6">
              ...
              <div className="bg-white dark:bg-secondary-800 rounded-lg p-6 shadow-sm border border-neutral-200 dark:border-secondary-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-secondary-900 dark:text-white">{selectedVerification.user.name}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-neutral-700 dark:text-neutral-300">Email</label>
                    <p className="text-secondary-900 dark:text-white">{selectedVerification.user.email}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-neutral-700 dark:text-neutral-300">Phone</label>
                    <p className="text-secondary-900 dark:text-white">{selectedVerification.user.phone || 'N/A'}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-neutral-700 dark:text-neutral-300">BVN</label>
                    <p className="text-secondary-900 dark:text-white font-mono">{selectedVerification.bvn}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-secondary-800 rounded-lg p-6 shadow-sm border border-neutral-200 dark:border-secondary-700">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="text-sm font-medium text-neutral-700 dark:text-neutral-300">Estimated Value</label>
                    <p className="text-secondary-900 dark:text-white font-semibold">
                  ...
                  <div>
                    ...
                    <p className="text-secondary-900 dark:text-white">{selectedVerification.description}</p>
                  </div>
                </div>
              </div>
              ...
              {activeTab === 'deleted' && selectedVerification && (
                <div className="bg-white dark:bg-secondary-800 rounded-lg p-6 shadow-sm border border-neutral-200 dark:border-secondary-700">
                  <h3 className="text-lg font-semibold text-secondary-900 dark:text-white mb-4 flex items-center">
                    <XCircle className="h-5 w-5 mr-2 text-neutral-600" />
                    Deletion Details
                  </h3>
                  <div className="space-y-3">
                    <div>
                      <label className="text-sm font-medium text-neutral-700 dark:text-neutral-300">Deleted At</label>
                      <p className="text-secondary-900 dark:text-white">{formatDate(selectedVerification.deletedAt)}</p>
                    </div>
                    {selectedVerification.deletedBy && (
                      <div>
                        <label className="text-sm font-medium text-neutral-700 dark:text-neutral-300">Deleted By</label>
                        <p className="text-secondary-900 dark:text-white">{selectedVerification.deletedBy.name}</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {activeTab === 'pending' && (
                <div className="bg-white dark:bg-secondary-800 rounded-lg p-6 shadow-sm border border-neutral-200 dark:border-secondary-700">
                <h3 className="text-lg font-semibold text-secondary-900 dark:text-white mb-4 flex items-center">
                  <MessageSquare className="h-5 w-5 mr-2 text-error" />
                  Review Decision
                </h3>
                ...
                <form onSubmit={handleReviewSubmit} className="space-y-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 border border-neutral-300 dark:border-neutral-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-primary-500 dark:bg-neutral-700 dark:text-white"
                    >
                  ...
                  <div className="flex justify-between">
                    <button
                      ...
                      className="px-4 py-2 bg-error text-white rounded-lg hover:bg-error transition-colors flex items-center space-x-2"
                    >
                    ...
                    <div className="flex space-x-4">
                      ...
                      <button
                        type="submit"
                        disabled={reviewing}
                        className="px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 disabled:bg-primary-400 disabled:cursor-not-allowed transition-colors flex items-center space-x-2"
                      >
                        {reviewing ? (
                          <>
            ...
            <div className="bg-white dark:bg-secondary-800 rounded-lg p-12 shadow-sm border border-neutral-200 dark:border-secondary-700 text-center">
