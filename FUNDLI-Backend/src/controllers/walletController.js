...
const verifyDeposit = async (req, res) => {
  try {
    ...
    if (paymentData.status === 'success') {
      ...
      wallet.updateBalance(transaction.amount, 'add');
    ...
    setTimeout(async () => {
      try {
        ...
        if (updatedTransaction && updatedTransaction.status === 'pending') {
          ...
          updatedWallet.updateBalance(amount, 'subtract');
...
const transferFunds = async (req, res) => {
  try {
    ...
    // Check limits
    const limitCheck = fromWallet.checkLimits('transfer', amount);
    if (!limitCheck.allowed) {
      return res.status(400).json({
        status: 'error',
        message: limitCheck.reason
    ...
    if (!toWallet) {
      toWallet = await Wallet.create({ user: toUserId, balance: 0, currency: fromWallet.currency });
    }
    ...
    fromWallet.updateBalance(amount, 'subtract');

    toWallet.addTransaction(toTransaction);
    toWallet.updateBalance(amount, 'add');
